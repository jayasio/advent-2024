...
const zeros: Vector[] = grid
  .flatMap((line, y) =>
    line.map((val, x) => (val === 0 ? new Vector(x, y) : null))
  )
  .filter((vec) => vec !== null);

function checkWithinBounds(vec: Vector) {
  return vec.x >= 0 && vec.x < grid.length && vec.y >= 0 &&
    vec.y < grid[0].length;
}
...
function getNextVecs(vec: Vector, grid: number[][]): Vector[] {
  return getNeighbors(vec).filter((v) => {
    return checkWithinBounds(v) &&
      getValueAt(v, grid) === getValueAt(vec, grid) + 1;
  });
}

function getPeaks(vec: Vector, grid: number[][]): Vector[] {
  return getNextVecs(vec, grid).flatMap((
    v,
  ) => (getValueAt(v, grid) === 9 ? [v] : getPeaks(v, grid)));
}

function getUnique(vecs: Vector[]): Vector[] {
  return [...new Set(vecs.map((v) => JSON.stringify(v)))].map((str) =>
    JSON.parse(str) as Vector
  );
}
